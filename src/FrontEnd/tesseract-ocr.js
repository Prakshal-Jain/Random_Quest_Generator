...
function progressUpdate(packet){
	...
	if(log.firstChild && log.firstChild.status === packet.status){
		...
		if(packet.status == 'done'){
			...
			let str = String(packet.data.text.replace(/\n\s*\n/g, '\n'))
			socket.emit("receive_question",str)
			document.getElementById("status").innerHTML = '<img src="completed.png" width="30px" height="30px"><b>Completed</b>'
...
function recognizeFile(file){
	...
	worker.recognize(file,
		...
		.progress(function(packet){
			console.info(packet)
			document.getElementById("status").innerHTML = "<img src = 'loading.gif' width='50px' height='50px'><b>Please wait. Processing Image: "+String((parseFloat(packet.progress)*100).toFixed(2))+"%</b>"
